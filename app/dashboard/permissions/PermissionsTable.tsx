...
function PermissionsTableInner<TData, TValue>({
  ...
  // drawer for Add New Group on small screens
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-end md:justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          {/* Use Sheet on small-and-up (sm+) like before */}
          <div className="hidden sm:block">
          ...
          {/* On small screens, use a Drawer (mobile-friendly) */}
          <div className="sm:hidden">
            <Drawer open={drawerAddOpen} onOpenChange={setDrawerAddOpen}>
              <DrawerTrigger asChild>
                <Button variant={"custom"}>
                  <CirclePlus
                    className="-ms-1 opacity-60"
      ...
      {applied ? (
        <>
          ...
          {/* Card list: shown on small and medium screens (below lg) */}
...
export default function PermissionsTable<TData, TValue>({
  ...
  // fetch (and localStorage merge) completes. This ensures the client's
  // sorting preferences are applied before the table renders, avoiding a
  // flash of default sorting. We always start in a loading state and clear
  // it only after the client fetch completes.
  ...
  // Only show the full-page loading skeleton while preferences are being
  // fetched client-side. Wait for preferences to be fetched and applied
  // before rendering the inner table so initial sorting is applied
  // without a flash (matches the UserTable behavior).
  if (loading) {
    return (
      <div className="bg-background overflow-hidden rounded-md border p-8">
        <div className="animate-pulse">
  ...
  return (
    <PermissionsTableInner
      ...
      // Only request the inner table wait for apply when we actually have
      // client-side preferences to apply. This ensures the inner table will
      // use a layout-effect to apply sorting before first paint.
      waitForApply={!!(loadedPrefs && Object.keys(loadedPrefs).length)}
    />
  );
}
